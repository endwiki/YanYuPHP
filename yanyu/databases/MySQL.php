...
use yanyu\Cache;
use yanyu\Config;
use yanyu\exceptions\CreateMySQLInstanceFailedException;
use yanyu\exceptions\DatabaseExecuteFailedException;
use yanyu\exceptions\DatabaseInsertDataHasEmptyException;
use yanyu\exceptions\DatabaseJoinTypeNotMissException;
use yanyu\exceptions\DatabaseTableNotMissException;
use yanyu\Logger;
...
class MySQL implements DatabaseInterface {
    ...
    public function fetch(bool $cache = false){
        ...
        // 如果存在缓存则读取缓存，否则查询
        if(Config::get('HAS_CACHE','DATABASE') && Cache::get(md5($sql))){
            return unserialize(Cache::get(md5($sql)));
        }else{
            // 查询数据
